...
function initTracker() {
    ...
    const dateSections = Array.from(document.querySelectorAll('article .tree-content h1'))
        .filter(el => {
    ...
    dateSections.forEach(section => {
        ...
        let count = 0;
        const headers = Array.from(document.querySelectorAll('article .tree-content h1'));
        const header = headers.find(h => h.textContent.includes(dateText));

        if (header) {
            const content = header.parentElement.querySelector('.markdownit') ||
