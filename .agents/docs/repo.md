...
`tests/test_ftip_prose.py`. The registry covers contextual authoring-language
families, including future
drafting instructions, document assembly, ownership and extraction narration,
workflow metaphors, and comparison worksheets. It is not limited to a fixed
list of reported examples, and it does not establish semantic prose quality.
Ordinary reader navigation, mathematical exposition, research terminology, and
experimental or runtime records remain legitimate. Each FTIP pull request still
needs a fresh independent
