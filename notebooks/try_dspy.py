...
def _(dspy, fd):
    ...
    pred1 = rag("What is the capital of France?", "France is a country in Europe.", _prediction=True)
...
def _(analyze_data, dataset, dspy):
    ...
    optimizer = dspy.SIMBA(metric=dataset.metric)
    compiled_analyze = optimizer.compile(analyze_data.module, trainset=dataset.train[10:42])
...
def _(analyze_data, analyze_optimized, dataset):
    for q in dataset.train[7:9]:
        print(f"{q['answer']}: {analyze_data(q['question'])}, {analyze_optimized(q['question'])}\n")
    return
