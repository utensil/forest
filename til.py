...
def extract_keywords_from_content(content):
    ...
    # Order matters for deterministic results - most important/specific first
    priority_keywords = [
        # Programming languages
        'rust', 'zig', 'elixir', 'clojure', 'javascript', 'typescript', 'python', 'c++', 'cpp', 'c', 'go', 'lean', 'apl', 'jank',
        # AI/ML specific
        'claude', 'dspy', 'textgrad', 'zenbase', 'simba', 'genai', 'reasoning', 'prompt', 'llm', 'ai', 'ml',
        # Systems/Performance
    ...
    # Look for priority keywords in order
    for keyword in priority_keywords:
        if keyword in content_lower:
            # Handle special cases for overlapping terms
            if keyword == 'c' and ('c++' in content_lower or 'cpp' in content_lower):
                continue  # Skip standalone 'c' if c++ is present
            if keyword == 'cpp' and 'c++' in found_keywords:
                continue  # Skip 'cpp' if 'c++' already found
            if keyword in ['ai', 'ml'] and any(term in found_keywords for term in ['genai', 'llm', 'dspy', 'claude']):
                continue  # Skip generic terms if specific ones present
                
            if keyword not in found_keywords:
                found_keywords.append(keyword)
    
    # Look for specific project/tool names mentioned (in deterministic order)
    project_patterns = [
