...
""",
    ...
    def test_broader_contextual_authoring_families_fail_in_both_modes(self) -> None:
        cases = (
            ...
            ("The remaining document clusters record the document sections before publication", "FTIP-DOCUMENT-ASSEMBLY"),
    ...
    def test_broader_research_and_navigation_counterexamples_pass(self) -> None:
        cases = (
            ...
            "The first document cluster separates astronomy papers from medical papers.",
            "The remaining document clusters record the same event from independent news sources.",
            "The verifier's evidence contract is the pair of a replay trace and its digest; acceptance requires both components.",
            "A harness lane is an isolated worker queue with its own state; two lanes cannot share mutable memory.",
